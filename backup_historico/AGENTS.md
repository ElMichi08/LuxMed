...
Fase de diseño, sin código todavía. Antes de proponer o escribir código, lee en este orden:
...
5. `CLAUDE.md` — invariantes de diseño y resumen de todo lo anterior.

No hay `package.json`, `pyproject.toml` ni comandos de build/lint/test todavía. La estructura de carpetas (`domain/`, `application/`, `infrastructure/`, `interface/`) ya está decidida — respétala en vez de improvisar una nueva.
...
- **Úsala siempre que modifiques o agregues un escenario** en `docs/BDD/` — varios ya están etiquetados `@borrador`/`@pendiente` (ver `docs/arquitectura.md`, sección de pendientes) y van a cambiar cuando el propietario los confirme. Escribir un escenario a mano sin la skill es la forma más fácil de romper la consistencia narrativa entre los 13 archivos.
