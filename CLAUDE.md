...
- `docs/BDD/*.feature` — especificación de comportamiento vigente y **única fuente de verdad del *qué***, un archivo por Feature. `Requisitos.MD` (v1.0, la fuente original) fue eliminado del repositorio: su contenido de comportamiento ya está cubierto y superado por `docs/BDD/` (sesión BDD 2026-08-12: consolidación de PDFs por paciente, exportación a Excel de resultados, resumen numérico del lote, y un modelo más preciso de los fallos del Portal 2); su Preámbulo de invariantes y Apéndice A de stack ya estaban duplicados más abajo en este archivo; su Apéndice B legal se rescató en `docs/legal.md`. Varios escenarios de `docs/BDD/` están etiquetados `@borrador` o `@pendiente` — no confirmados aún por el propietario (el caso más importante: el comportamiento de fallo del Portal 3 en `06_consulta_portal3.feature` es una extrapolación por analogía sin validar, marcada explícitamente como no confiable).
