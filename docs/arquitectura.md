...
> Las features 05, 11, 12 y 13 de `docs/BDD/` fueron confirmadas explícitamente por el propietario el 2026-08-15 y ya no llevan `@borrador`. Solo `docs/BDD/06_consulta_portal3.feature` sigue con escenarios `@pendiente` — su comportamiento de fallo es una extrapolación por analogía que el propietario todavía no revisó contra el portal real, y quedó fuera de ese cierre a propósito. No construir el adapter de Portal 3 ni fijar `ERROR_PORTAL_3` en el dominio hasta explorar ese portal en otra sesión.
...
### Configuración de URLs de portales (nuevo, 2026-08-15)

Las URLs de los 3 portales **no van hardcodeadas** dentro de `infrastructure/playwright/portal*_adapter.py`, sino externalizadas como variables de entorno (`.env` local, en `.gitignore`, con un `.env.example` versionado como plantilla sin valores reales).

- **Motivo no es ocultar un secreto** (invariante #5 ya cubre lo único genuinamente sensible: las credenciales del Portal 3, que el sistema nunca recibe ni almacena). Los portales son sitios públicos gubernamentales. El motivo es no dejar explícito en el código fuente — de un repositorio que puede ser público — exactamente qué sitios gubernamentales se automatizan, y poder cambiar una URL si un portal migra de dominio sin tocar código ni re-testear nada.
- Se cargan **una sola vez en `main.py`** (composition root) y se inyectan a cada adapter concreto por constructor. Nunca se leen variables de entorno directamente dentro de `infrastructure/playwright/*` (ni mucho menos en `domain`/`application`) — mantiene la config centralizada y no rompe la regla de dependencia hacia adentro.
- Si al arrancar falta una URL o está vacía, el fallo debe ser inmediato y explícito (error claro en `main.py` antes de lanzar el primer worker) — nunca un fallo silencioso a mitad del lote por una URL vacía.
- **Pendiente de implementar** (ver sección "Pendiente" al final): `.env.example` + un módulo de carga simple (`config.py`, sin librería adicional más allá de quizás `python-dotenv`) — se resuelve al atacar la rama `infrastructure/playwright`, junto con el resto de los adapters concretos.
...
## CI / Pipeline (actualizado, 2026-08-20)

- **Un solo workflow, `.github/workflows/tests.yml`**, con dos jobs:
  1. `static-analysis`: Corre en Ubuntu y ejecuta `ruff check .`, `ruff format --check .` y `mypy domain application tests`.
  2. `pytest`: Corre tras el análisis estático en una matriz de SO (`ubuntu-latest` y `windows-latest`) ejecutando `pytest tests/ -v`, validando lógica y compatibilidad con el entorno de destino (Windows). Incluye pruebas automatizadas de frontera arquitectónica (`test_architecture_boundaries.py`) para asegurar la pureza de `domain` y `application`.
- **Vive en `main`** (es config transversal, igual que `.gitignore` o un futuro `pyproject.toml` — ver `CLAUDE.md`, "Convención de ramas y commits"). Se agrega ahí y se trae a las ramas activas con `git merge main` (nunca rebase), el mismo patrón ya usado en `domain` para traer cambios compartidos sin mezclarlos con el historial de una rama de capa.
- **Dispara solo en `main` y `test`** (`push` y `pull_request`), con control de concurrencia (`concurrency: cancel-in-progress: true`) para evitar ejecuciones redundantes. Las ramas de capa (`domain`, `application`, `infrastructure/*`, `interface`) no corren CI por sí solas — se validan cuando se mergean a `test`, que es donde ya se hace la revisión manual antes de `main`.
- **Instala dependencias con `pip install pytest ruff mypy` a pelo**, porque no existe `pyproject.toml`/`requirements.txt` todavía y `domain`/`application` solo usan la stdlib. Hay que actualizar este paso cuando se cree el packaging real — las ramas de `infrastructure/*` van a necesitar Playwright, pandas, pikepdf, etc., que este workflow todavía no instala.
- **El workflow por sí solo no bloquea nada** — solo reporta rojo/verde en la pestaña Actions. Bloquear el merge de verdad requeriría una branch protection rule ("Require status checks to pass before merging"), pero GitHub no la ofrece en repos privados sin plan pago — **decidido 2026-08-17: no se paga el upgrade ni se hace público el repo solo por esto** (ver "Pendiente" abajo). El gate es disciplina manual: no mergear `test` → `main` hasta ver el check en verde, apoyado por `.github/pull_request_template.md`.
...
1. **Heterogeneidad genuina entre los 3 portales.** Portal 1 usa interceptación de respuesta; Portal 2 tiene ALTCHA con proof-of-work y **3 desenlaces de negocio distintos** (cobertura vigente, `SIN_COBERTURA_PORTAL_2`, PDF corrupto) más **2 ciclos de reintento + intervención humana independientes** (comportamiento confirmado por el propietario el 2026-08-15); Portal 3 reutiliza una sesión persistente en memoria. "Un gateway por portal" evita forzar esto a un `PortalClient` genérico con parámetros opcionales e ifs internos.
...
- ~~Habilitar branch protection en GitHub para `main` exigiendo el check `pytest`~~ — **decidido 2026-08-17: no se hace.** GitHub no permite branch protection rules ni rulesets en repos privados sin plan pago (Pro o superior); el repositorio se mantiene privado (los docs de diseño no tienen nada crítico committeado hoy, pero el proyecto está pensado con retención cero y no vale la pena hacerlo público solo por esto). El pipeline de `.github/workflows/tests.yml` (ver "CI / Pipeline" arriba) sigue corriendo en cada push/PR a `main`/`test` y reporta rojo/verde en la pestaña Actions — el gate es disciplina manual (no mergear `test` → `main` hasta ver el check en verde), no un bloqueo forzado por GitHub. Si el equipo crece o se justifica el gasto, revisar upgrade a GitHub Pro.
...
- ~~Confirmación explícita del propietario sobre el mapeo estado→texto de Excel~~ — **resuelto 2026-08-15**: el propietario confirmó el mapeo completo de `12_exportacion_excel_resultado.feature`, incluyendo que "Error Portal 2" agrupa `ERROR_PORTAL_2` y `PDF_CORRUPTO_PORTAL_2`. Sigue condicional solo en la fila `ERROR_PORTAL_3` (ver punto de Portal 3 arriba).
- ~~Confirmación explícita del propietario sobre el ciclo completo de Portal 2~~ — **resuelto 2026-08-15**: el propietario confirmó el ciclo de ALTCHA (3 intentos automáticos + intervención humana + decisión del médico de reintentar o marcar `ERROR_PORTAL_2`) y el ciclo de PDF corrupto del titular tal como quedaron redactados en `05_consulta_portal2.feature`, ya sin `@borrador`.
- **Confirmación de que Rama A sin cédula del titular omite Portal 3** — asunción documentada en `docs/BDD/06_...` y ya reflejada en la orquestación (`process_patient.py`), pero sin validar explícitamente por el propietario tras introducir los 3 estados nuevos de Portal 2. Sigue abierto — depende de Portal 3, fuera del alcance del cierre del 2026-08-15.
